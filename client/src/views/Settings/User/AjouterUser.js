...
import { userAdded, userGetById, verifEmail } from "../../../Redux/usersReduce";
...
function AjouterUser() {
  ...
  async function submitForm(event) {
    ...
    if (tel !== "" && !validateTel()) {
      notify(2, 'Numero téléphone est invalide');
      return;
    }

    if (email !== "" && !validateMail()) {
      notify(2, 'Adresse email est invalide');
      return;
    }
    ...
    if (!validator.isEmpty(nom) && validator.isEmail(email) && !validator.isEmpty(login) && testPassword === true &&
      ...
      dispatch(userAdded({ nom, email, tel, login, password, id, etat, role })).then(data => {
  ...
  const checkEmail = async () => {
    try {
      var response = await dispatch(verifEmail({ id: id, email: email }));
      if (response.payload === true) {
        notify(2, "Adresse email existe déja");
        return true;
      }
      return false;
    } catch (error) {
